...
class DropdownBootstrap extends Component {
    ...
    this.state = {
      ...
      steps: [
        ...
        },
        {
          element: ".step2",
          intro:
            "Choose a demographic here to see data only for that group. Choose everyone if you want to see the distribution for the whole population.",
        },
        {
          element: ".step3",
          intro:
            "Choose one of these options to see data for the Black Population or the white population.",
        },
        {
          element: ".step4",
        ...
        },
        {
          element: ".step5",
          intro:
            "Choose one of these if you are interested in a subgroup of race and gender. Not all groups are available for all variables. Buttons will gray out if they are not available for certain variables.",
        },
        {
          element: ".step6",
          intro:
            "Explore a map showing the distribution of the variable by County. Hover over a county to see its exact value.",
          position: "right",
        },
        {
          element: ".step7",
          intro:
            "This is a dotplot showing the distribution of the variable. Each circle corresponds to a single county. Each stack of circles corresponds to a range of values. Hover over a circle to see its exact value. Note that the axis does not always start at 0!",
          position: "left",
        },
        {
          element: ".step8",
          intro:
            "Get a quick summary of the variable by seeing the top 10 counties and their values here.",
          position: "left",
        },
        {
          element: ".step9",
      ...
      <Row className="justify-content-center">
        <Grid className="step2" container xs={8} sm={8} md={6} lg={4}>
          ...
          <Grid container className="RaceGender step5" sm={8}>
            {this.demListToButton(
              ["WhiteWomen", "BlackWomen", "WhiteMen", "BlackMen"],
              6,
              6
            )}
          </Grid>
  ...
  getabbreviation(matchingvar, demselected, genderselected) {
    ...
    abbrev =
      matchingvar.abbreviation + "_" + genderselected + "_" + demselected;
    // var genderabbrev = "";
    // var totalabbrev = "";
  ...
  handleClick(index, i, event) {
    ...
    var error = false;
    if (
      this.state.genderselected !== "A" &&
      this.state.demselected !== "A" &&
      matchingvar.racegender === false
    ) {
      error = true;
    ...
    if (error === true) {
      ...
      document
        .getElementsByClassName(["Everyone"])[0]
        .classList.add("selected");
      buttonselected = "Everyone";
    }
    var abbreviation = this.getabbreviation(
      matchingvar,
      demselected,
      genderselected
    );
    ...
    this.setState({
      childcare: false,
      currentvar: true,
      varlocation: { index1: index, index2: i },
      varname: matchingvar.name,
      varabbreviation: abbreviation,
      race: matchingvar.race,
      gender: matchingvar.gender,
      racegender: matchingvar.racegender,
      demselected: demselected,
  ...
  handleDemClick(event) {
    if (this.state.currentvar) {
      ...
      this.setState({
        demselected: event.target.value[0],
        genderselected: event.target.value[2],
        varabbreviation: abbreviation,
        varname: matchingvar.name,
        buttonselected: event.target.value.slice(4),
      });
    } else {
      this.setState({
        demselected: event.target.value[0],
